...
+icon_tag :search, { id: "search", name: "search_icon" }+ produces:

  <i id="search" name="search_icon", class="icon-search"></i>

+icon_tag :search+ produces:
...
- +pull+ pulls the icon to the left or right.  Adds +pull-left+ or +pull-right+ to the class when the value is set to +left+ or +right+.  Accepts a string.
- +spin+ spins the icon.  Adds +icon-spin+ to the class when the value is set to +true+.  Accepts a boolean.
...
- +rotate+ rotes the icon the specified number of degrees.  Adds +icon-rotate-90+, +icon-rotate-180+ or +icon-rotate-270+ to the class when the value is set to +90+, +180+ or +270+.  Accepts an integer.
